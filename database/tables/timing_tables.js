...
export function seedTimingTables(db, only_if_empty = true) {
    ...
    db.transaction(tx => {
        tx.executeSql(
            ...
            [],
            (_, {rows: {_array}}) => {
                // if the session table is empty or only_if_empty is false, seed the tables
                if (_array[0]["COUNT(*)"] === 0 || !only_if_empty) {
                    ...
                    for (let i = 0; i < no_of_days; i++) {
                        ...
                        while (start_date.getHours() < end_of_day) {
                            ...
                            const duration = Math.min(Math.max(Math.round(faker.datatype.number({
                                min: 90,
                                max: 120
                            })), 15), 120);
                            ...
                            if (planned_end_date < new Date() && Math.random() < 0.7) {
                                ...
                                actual_duration = Math.round(faker.datatype.number({
                                    min: duration - 30,
                                    max: duration + 30
                                }));
                    ...
                    console.log("Timing tables already seeded");
                }
            },
            (error) => console.log(error)
        );
